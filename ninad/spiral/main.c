// Archimedean spiral (r = a + b*theta), rendered as an ASCII spider web over
// UART, on bare-metal TinyRocket. Same "no hardware FPU" situation as
// mandelbrot/main.c -- ISA is rv32imac, no F/D extension -- so this uses
// Q8.8 fixed-point (8 integer bits, 8 fractional bits) throughout, including
// for sin/cos.
//
// A real spider's orb web is exactly this shape: straight radial spokes
// from the hub, plus one continuous spiral thread wound around them -- so
// the "novelty" here isn't bolted on, it's the same math the picture is
// named after.
